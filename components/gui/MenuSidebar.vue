  ...
  .menu-container(ref="menu_container" v-bind:class="style_name" name="menu-container")
    ...
    .menu-options-container(v-if="type === 'hover'" v-bind:class="{ isin: revealMenu }")
      .menu-option-group(v-for="(pm, index) in options" :name="pm.name" :key="`pm-${index}`" v-bind:class="`pie${index+1}`")
        //- - var disabled = mtd.disabled;
        .menu-option(v-for="(mtd, index) in pm.methods" :key="`mtd-${index}`" @click="commandClicked" :name="`${mtd.name}`" v-bind:class="[`${pm.name} ${mtd.name}`, {disabled: mtd.disabled}]")
    //- If it is an info menu
  ...
  .menu-options-container {
    ...
    .menu-option {
      ...
      &.env {
        ...
        &.spotlight {
          @include makeBackground('/icons/icon-spotlight-on.svg', 24px);
          &.off {
            @include makeBackground('/icons/icon-spotlight-off.svg', 24px);
          }
        } 
      }
    }
